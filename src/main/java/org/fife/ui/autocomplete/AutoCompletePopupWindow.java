...
class AutoCompletePopupWindow extends JWindow implements CaretListener, ListSelectionListener, MouseListener {
	 ...
	 */
	protected CompletionListModel model;
	 ...
	 */
	protected Completion lastSelection;

	/**
	 * Optional popup window containing a description of the currently selected
	 * completion.
	 */
	protected AutoCompleteDescWindow descWindow;
	 ...
	 */
	protected Dimension preferredDescWindowSize;

	/**
	 * Whether the completion window and the optional description window should
	 * be displayed above the current caret position (as opposed to underneath
	 * it, which is preferred unless there is not enough space).
	 */
	protected boolean aboveCaret;

	protected int lastLine;

	protected boolean keyBindingsInstalled;

	protected KeyActionPair escapeKap;
	protected KeyActionPair upKap;
	protected KeyActionPair downKap;
	protected KeyActionPair ctrlUpKap;
	protected KeyActionPair ctrlDownKap;
	protected KeyActionPair leftKap;
	protected KeyActionPair rightKap;
	protected KeyActionPair enterKap;
	protected KeyActionPair tabKap;
	protected KeyActionPair homeKap;
	protected KeyActionPair endKap;
	protected KeyActionPair pageUpKap;
	protected KeyActionPair pageDownKap;
	protected KeyActionPair ctrlPageUpKap;
	protected KeyActionPair ctrlPageDownKap;
	protected KeyActionPair ctrlCKap;

	protected KeyActionPair oldEscape, oldUp, oldDown, oldCtrlUp, oldCtrlDown, oldLeft, oldRight, oldEnter, oldTab,
			oldHome, oldEnd, oldPageUp, oldPageDown, oldCtrlPageUp, oldCtrlPageDown, oldCtrlC;

	/**
	 * The space between the caret and the completion popup.
	 */
	protected static final int VERTICAL_SPACE = 1;

	/**
	 * The class name of the Substance List UI.
	 */
	protected static final String SUBSTANCE_LIST_UI = "org.pushingpixels.substance.internal.ui.SubstanceListUI";
	 ...
	 */
	protected AutoCompleteDescWindow createDescriptionWindow() {
	 ...
	 */
	protected static final KeyStroke getCopyKeyStroke() {
	...
	protected void createKeyActionPairs() {
		...
		oldCtrlPageUp = new KeyActionPair();
		oldCtrlPageDown = new KeyActionPair();
	...
	protected void internalCreateKeyActionPairs(EnterAction enterAction) {
		...
		ctrlPageUpKap = new KeyActionPair("PageUp", new PageUpAction());
		ctrlPageDownKap = new KeyActionPair("PageDown", new PageDownAction());
	...
	protected void installKeyBindings() {
		...
		replaceAction(im, am, KeyEvent.VK_UP, InputEvent.CTRL_DOWN_MASK, ctrlUpKap, oldCtrlUp);
		replaceAction(im, am, KeyEvent.VK_LEFT, leftKap, oldLeft);
		replaceAction(im, am, KeyEvent.VK_DOWN, downKap, oldDown);
		replaceAction(im, am, KeyEvent.VK_DOWN, InputEvent.CTRL_DOWN_MASK, ctrlDownKap, oldCtrlDown);
		...
		replaceAction(im, am, KeyEvent.VK_PAGE_UP, InputEvent.CTRL_DOWN_MASK, ctrlPageUpKap, oldCtrlPageUp);
		replaceAction(im, am, KeyEvent.VK_PAGE_DOWN, pageDownKap, oldPageDown);
		replaceAction(im, am, KeyEvent.VK_PAGE_DOWN, InputEvent.CTRL_DOWN_MASK, ctrlPageDownKap, oldCtrlPageDown);
	...
	protected void uninstallKeyBindings() {
		...
		putBackAction(im, am, KeyEvent.VK_UP, InputEvent.CTRL_DOWN_MASK, oldCtrlUp);
		putBackAction(im, am, KeyEvent.VK_DOWN, oldDown);
		putBackAction(im, am, KeyEvent.VK_DOWN, InputEvent.CTRL_DOWN_MASK, oldCtrlDown);
		...
		putBackAction(im, am, KeyEvent.VK_PAGE_DOWN, InputEvent.CTRL_DOWN_MASK, oldCtrlPageDown);
	 ...
	 * window to fit.
	 */
	protected void positionDescWindow() {
	 ...
	 *      KeyActionPair)
	 */
	protected void putBackAction(InputMap im, ActionMap am, int key, int modifier, KeyActionPair kap) {
	...
	protected void putBackAction(InputMap im, ActionMap am, int key, KeyActionPair kap) {
		putBackAction(im, am, key, 0, kap);
	}
	 ...
	 */
	protected void replaceAction(InputMap im, ActionMap am, int key, KeyActionPair kap, KeyActionPair old) {
		replaceAction(im, am, key, 0, kap, old);
	}

	protected void replaceAction(InputMap im, ActionMap am, int key, int mask, KeyActionPair newKap,
			KeyActionPair oldKap) {
		KeyStroke keyStroke = KeyStroke.getKeyStroke(key, mask);
		oldKap.key = im.get(keyStroke);
		im.put(keyStroke, newKap.key);
		oldKap.action = am.get(newKap.key);
		am.put(newKap.key, newKap.action);
	}
	 ...
	 */
	protected void selectPageDownItem() {
	 ...
	 */
	protected void selectPageUpItem() {
		int visibleRowCount = list.getVisibleRowCount();
	 ...
	 */
	protected static class KeyActionPair {
	 ...
	 */
	protected class PopupList extends JList {
		private static final long serialVersionUID = 1L;
